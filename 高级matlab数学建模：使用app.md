...
在R2022a版本中最好只用数据处理器处理数值类型数据，字符串类型标签可以转化成数值类型进行标记。下面仅做简要演示(数据集:\shumo\zhongyuan_defaults.csv)

文本类标签数据的常用方法(python):

```python
mapping={...}#设置要替换的字符串为key值，替换的数值为value
...
观察”work_year"列存在缺失数据，在清理方法中选择清理缺失数据进行清理
...
代价选项是设置罚值，即分类错误的情况下调整模型，使其接近真实值。

部分分类模型可以设置优化器，在分类过程中加入优化模型提高精度，再次不做说明。

步骤3：选择分类模型

提供了众多分类模型，如常见的决策树、逻辑回归、贝叶斯判别等。
...
回归学习器也是类似的用法，只不过待预测值从离散值变成了连续值，在此不做说明。
