...
本教程更多注重于matlab语法和常用函数，编程逻辑需要读者自行练习积累。
...
如果不初始化，在计算过程中程序会自动分配内存大小，数据量较小时运行速度慢，数据量大时会报错。
...
>> randi([2,15],3,2) %生成一定范围内随机整数，[2,15]表示生成包括2到15在内的整数，3，2表示生成矩阵的行和列
...
同理可以使用isnan，isinf函数判断一个数是否为非数值或者无穷。
